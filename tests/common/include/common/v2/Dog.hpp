 ...
 */
#ifndef COH_TEST_V2_DOG_HPP
#define COH_TEST_V2_DOG_HPP
...
class Dog
    ...
    protected:
        ...
        Dog(String::View vsName, int m_cAge, String::View vsBreed, Color::View vColor)
            : super("Canis lupus familiaris", vsName, m_cAge),
              f_vsBreed(self(), vsBreed),
              f_vColor(self(), vColor),
              f_hEvolvable(self(), SimpleEvolvable::create(2))
            {
            }
    ...
    public:
        ...
        virtual Color::View getColor() const
            {
            return f_vColor;
            }

        virtual void setColor(Color::View vColor)
            {
            initialize(f_vColor, vColor);
            }
    ...
    public:
        virtual void readExternal(PofReader::Handle hIn)
            ...
            if (s_nTypeId == hIn->getUserTypeId())
                ...
                if (hIn->getVersionId() >= 2)
                    {
                    initialize(f_vColor, cast<Color::View>(hIn->readObject(1)));
                    }
                }
            else
                {
    ...
    public:
        virtual size32_t hashCode() const
            ...
            result = 31 * result + f_vColor->hashCode();
            return result;
            }

        virtual TypedHandle<const String> toString() const
            {
            return COH_TO_STRING("Dog.v2{" <<
                   "name='" << f_vsName << '\''
                   << ", age=" << m_cAge
                   << ", species=" << f_vsSpecies
                   << ", breed='" << f_vsBreed << '\''
...
COH_REGISTER_PORTABLE_TYPE_CLASS(Dog::s_nTypeId, Dog);

COH_CLOSE_NAMESPACE3

#endif // COH_TEST_V2_DOG_HPP
